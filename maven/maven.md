...
项目构建：提供标准的、跨平台自动化项目构建方式

依赖管理：快捷管理项目依赖，避免版本冲突问题
...
- 组成部分
  ...
  - groupId：定义当前项目隶属组织名称，通常是反写域名
  - artifactId：定义项目名称，通常是模块名称
  - version：项目版本号
...
`mvn install`：把打包项目放到本地仓库

## 依赖管理

### 依赖具有传递性

- 直接依赖：项目中直接配置的建立的依赖关系
- 间接依赖：配置资源的依赖关系会被传递到当前项目中

### 依赖传递冲突问题

- 路径优先：层级越深，优先级越低
- 声明优先：层级相同，配置顺序靠前覆盖顺序靠后
...
依赖默认情况是可以在任何地方使用，可以通过scope标签设置作用范围
...
- 相同点
  ...
  - 聚合与继承均属于设计型模块，没有实际的内容
- 不通点
  - 聚合是在当前模块中配置关系，聚合可以感知有哪些聚合模块
  - 继承是在子模块中配置关系，父模块无法感知有哪些子模块继承了自己
...
`${settings.localRepository}`：setting标签中的值
